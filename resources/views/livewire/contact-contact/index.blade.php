                ...
                <tr>
                    ...
                    <td>
                        @if ($contactContact->company)
                            <span
                                class="badge badge-relationship">{{ $contactContact->company->company_name ?? '' }}</span>
                        @endif
                    </td>
                    <td>
                        ...
                        <div class="flex justify-end">
                            @can('contact_contact_show')
                                <a class="btn btn-sm btn-info mr-2"
                                    href="{{ route('admin.contact-contacts.show', $contactContact) }}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('contact_contact_edit')
                                <a class="btn btn-sm btn-success mr-2"
                                    href="{{ route('admin.contact-contacts.edit', $contactContact) }}">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan
                            @can('contact_contact_delete')
                                <button class="btn btn-sm btn-rose mr-2" type="button"
                                    wire:click="confirm('delete', {{ $contactContact->id }})" wire:loading.attr="disabled">
                                    {{ trans('global.delete') }}
                                </button>
                            @endcan
                        </div>
