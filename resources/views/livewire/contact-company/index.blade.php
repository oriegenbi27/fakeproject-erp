                        ...
                        <div class="flex justify-end">
                            @can('contact_company_show')
                                <a class="btn btn-sm btn-info mr-2"
                                    href="{{ route('admin.contact-companies.show', $contactCompany) }}">
                                    {{ trans('global.view') }}
                                </a>
                            @endcan
                            @can('contact_company_edit')
                                <a class="btn btn-sm btn-success mr-2"
                                    href="{{ route('admin.contact-companies.edit', $contactCompany) }}">
                                    {{ trans('global.edit') }}
                                </a>
                            @endcan
                            @can('contact_company_delete')
                                <button class="btn btn-sm btn-rose mr-2" type="button"
                                    wire:click="confirm('delete', {{ $contactCompany->id }})" wire:loading.attr="disabled">
                                    {{ trans('global.delete') }}
                                </button>
                            @endcan
                        </div>
