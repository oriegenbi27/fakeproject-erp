<form wire:submit.prevent="submit" class="pt-3">
    ...
    <div class="form-group {{ $errors->has('transaction.amount') ? 'invalid' : '' }}">
        ...
        <input class="form-control" type="number" name="amount" id="amount" wire:model.defer="transaction.amount"
            step="0.01">
    ...
    <div class="form-group {{ $errors->has('transaction.transaction_date') ? 'invalid' : '' }}">
        <label class="form-label"
            for="transaction_date">{{ trans('cruds.transaction.fields.transaction_date') }}</label>
        <x-date-picker class="form-control" wire:model="transaction.transaction_date" id="transaction_date"
            name="transaction_date" picker="date" />
        <div class="validation-message">
